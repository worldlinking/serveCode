...
def histogram_match(source, reference, match_proportion=1.0):
    ...
    """
    orig_shape = source.shape  # 获取数组的行和列
    source = source.ravel()  # 将数组维度拉成一维数组

    if np.ma.is_masked(reference):  # 确定输入是否有屏蔽值
    ...
    # get the set of unique pixel values 获取一组唯一的像素值
    # and their corresponding indices and counts 以及相应的指数和计数
    ...
    # take the cumsum of the counts; empirical cumulative distribution 取计数的总和；经验累计分布
    ...
    # find values in the reference corresponding to the quantiles in the source 在参考中查找与源中的分位数相对应的值
    ...
    # using the inverted source indicies, pull out the interpolated pixel values 使用反转的源索引，取出插值像素值
    logger.debug("create target array from interpolated values by index")
    target = interp_r_values[s_idx]

    # interpolation b/t target and source 插入b/t目标和源
